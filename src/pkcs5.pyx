...
cpdef bytes pkcs5_alg1(str password, str salt, int iteration_count, hash):
    if len(salt) != 8:
...
cpdef bytes pkcs5_alg2(str password, str salt, int iteration_count, hash, unsigned long outlen):  
    cdef HashDescriptor desc = HashDescriptor(hash)
    if not outlen:
...
cpdef bytes pkcs5(str password, str salt='', int iteration_count=1024, hash='sha256', unsigned long length=0):
